                    ...
                    <div class="row g-0 p-lg-3">
                        ...
                        <div class="col-lg-9 px-lg-4 mt-lg-5">
                            <div class="card-body">
                                ...
                                <p class="card-text">${news.details.replace(/^(.{400}[^\s]*).*/,"$1") + "..."}</p>
                                <div class = "d-flex flex-wrap justify-content-between gap-lg-5 mt-lg-5">
                                    <div class = "d-flex gap-2">
                                        ...
                                        <div>
                                        <h6 class="mb-lg-0">${news.author.name ? news.author.name : "--"}</h6>
                                        <p class="mb-lg-0">${news.author.published_date ? news.author.published_date : "--"}</p>
                                        </div>
                                    </div>   
                                    <div class = "d-flex gap-lg-2 ms-lg-5 mt-lg-3">
                                        ...
                                        <h5 class="mt-1">${news.total_view ? news.total_view : "No views"}</h5>
                                    </div>
        ...
        card.addEventListener('click', () => {
            fetch(`https://openapi.programming-hero.com/api/news/${card.id}`)
                ...
                .then(data => {
                    ...
                    authorName.innerHTML = data.data[0].author.name ? data.data[0].author.name : "No name found";
                    newsContent.innerHTML = data.data[0].details ? data.data[0].details : "No news found";
                    newsTitle.innerHTML = data.data[0].title ? data.data[0].title : "No title found";
                })
            .catch(error => console.log(error))
        })
    })
